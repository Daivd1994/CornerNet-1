...
def top_k(heat,k=100):
    ...
    k_class=math.cile(k_index/(h*w))-1
    k_position=k_index%(h*w)
    k_y=math.cile(k_position/w)-1#0 is also a cata
    k_x=k_position%w
...
def map_to_vector(feature_map,inds,transpose=True):
    ...
    if transpose:
        inter_vector=tf.reshape(feature_map,(tf.shape(feature_map)[0],-1,tf.shape(feature_map)[-1]))
    else:
...
def expand_copy(feature_map,k,inter=False):
    ...
    temp=feature_map
    for i in range(k-1):
        temp=tf.concat(-1,[temp,feature_map])
    if inter:
