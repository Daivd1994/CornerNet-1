...
class Model():
    def conv_bn_re(self,inputs,input_dim,out_dim,strides=(1,1),use_relu=True,use_bn=True,k=3,is_training=True,scope='conv_bn_re'):
        with tf.variable_scope(scope):
            x=tf.layers.conv2d(inputs,out_dim,k,strides=strides,padding='same',kernel_initializer=initializers.xavier_initializer())
            if use_bn:
    ...
    def hourglass(self,inputs,n_deep,n_res,n_dims,is_training=True,scope='hourglass_5'):
        with tf.variable_scope(scope):
            ...
            up_1=self.res_block(inputs,curr_dim,next_dim,curr_res,is_training=is_training,scope='up_1')

            half=tf.nn.max_pool(inputs,ksize=[1,2,2,1],strides=[1,2,2,1])
            low_1=self.res_block(half,curr_dim,next_dim,curr_res,is_training=is_training,scope='low_1')
            if n_deep>1:
                low_2=hourglass(low_1,n_deep-1,n_res[1:],n_dims[1:],is_training=is_training,scope='hourglass_%d'%(n_deep-1))
            else:
    ...
    def heat(self,inputs,input_dim,out_dim,scope='heat'):
        ...
        with tf.variable_scope(scope):
            ...
            x=tf.contrib.layers.conv2d(x,out_dim,1)
            return x
