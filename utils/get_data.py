...
class Image_data():
    ...
    def read_from_disk(self,queue):
        ...
        tl_heatmaps = np.zeros((self.output_size[0], self.output_size[1],self.categories), dtype=np.float32)
        br_heatmaps = np.zeros((self.output_size[0], self.output_size[1],self.categories), dtype=np.float32)
        ...
        return image, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes

    def get_single_data(self,queue):
        images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes=tf.py_func(self.read_from_disk,[queue],
            [tf.float32,tf.int64,tf.int64,tf.float32,tf.float32,tf.uint8,tf.float32,tf.float32,tf.int64])
        return images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes
    def inupt_producer(self):
        quene_train=tf.train.slice_input_producer([self.coco.get_all_img()],shuffle=True)
        self.images, self.tl_tags, self.br_tags,self.tl_heatmaps, self.br_heatmaps, self.tag_masks, self.tl_regrs, self.br_regrs,self.boxes=self.get_single_data(quene_train)
    def get_batch_data(self,batch_size):
        images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes=tf.train.shuffle_batch([self.images,
            self.tl_tags, self.br_tags,self.tl_heatmaps, self.br_heatmaps, self.tag_masks, self.tl_regrs, self.br_regrs,self.boxes],
            ...
            (128),(128,2),(128,2),(128,4)],capacity=100,min_after_dequeue=batch_size,num_threads=16)
        return images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes

if __name__=='__main__':
    data=Image_data('trainval')
    data.inupt_producer()
    images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes=data.get_batch_data(2)
    sess=tf.InteractiveSession()
    coord = tf.train.Coordinator()
    threads=tf.train.start_queue_runners(sess=sess,coord=coord)
    for i in range(12):
        images_, tl_tags_, br_tags_,tl_heatmaps_, br_heatmaps_, tag_masks_, tl_regrs_, br_regrs_,boxes_= sess.run([images, tl_tags, br_tags,tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs,boxes])
        for j in range(2):
            print(images_.shape,tl_tags_.shape,tl_heatmaps_.shape,tag_masks_.shape,tl_regrs_.shape)
            img=(images_[j]*255).astype(np.uint8)
            heat_1=np.max(tl_heatmaps_[j],axis=-1)
            print('kkkkkkk')
            ...
            heat_2=np.max(br_heatmaps_[j],axis=-1)*255
